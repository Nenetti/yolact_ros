...
namespace semantic_segmentation {
    ...
    using semantic_segmentation::Filter;
    using semantic_segmentation::Segment;
    using semantic_segmentation::Cluster;
    ...
    void SemanticMapping::to_segmentation(PointCloud<PointXYZRGB> &cloud, std::vector<Segment> &segments, std::vector<Cluster> &clusters) {
        ...
        set_color(segments);
        remove_unavailable_segments(segments);
    ...
    void SemanticMapping::remove_unavailable_segments(std::vector<Segment> &segments) {
        int i = 0;
        int size = int(segments.size());
        while (i < size) {
            if (!segments[i].is_available) {
