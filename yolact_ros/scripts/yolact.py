...
def make_net(in_channels, conf, include_last_relu=True):
    ...
    def make_layer(layer_cfg):
        ...
        if isinstance(layer_cfg[0], str):
            ...
            if layer_name == 'cat':
                nets = [make_net(param["in_channels"], x) for x in layer_cfg[1]]
            ...
            if kernel_size > 0:
                layer = nn.Conv2d(param["in_channels"], num_channels, kernel_size, **layer_cfg[2])
            else:
                if num_channels is None:
                    layer = InterpolateModule(scale_factor=-kernel_size, mode='bilinear', align_corners=False, **layer_cfg[2])
                else:
                    layer = nn.ConvTranspose2d(param["in_channels"], num_channels, -kernel_size, **layer_cfg[2])
        param["in_channels"] = num_channels if num_channels is not None else param["in_channels"]
    ...
    return nn.Sequential(*(net)), param["in_channels"]
