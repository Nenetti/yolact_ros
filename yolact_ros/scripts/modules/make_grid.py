...
def test_uniqueness(grids):
    ...
    for y in range(grid_h):
        for x in range(grid_h):
            ...
            l1_dist = np.sum(np.abs(grids - np.tile(pixel_features, grid_h * grid_w).reshape((-1, grid_h, grid_w))),
                             axis=0)
            ...
            if num_equal > 1:
                print('Pixel at (%d, %d) has %d other pixel%s with the same representation.' % (
                x, y, num_equal - 1, '' if num_equal == 2 else 's'))
                return False
